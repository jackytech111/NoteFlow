...
export const validateTagsSchema = Joi.object({
  tagIds: Joi.array().items(Joi.string().uuid()).min(1).optional().messages({
    "array.base": "Tag IDs must be an array",
    "array.min": "At least one tag ID is required",
    "string.uuid": "Each tag ID must be a valid UUID",
  }),
  tagsIds: Joi.array().items(Joi.string().uuid()).min(1).optional().messages({
    "array.base": "Tag IDs must be an array",
    "array.min": "At least one tag ID is required",
    "string.uuid": "Each tag ID must be a valid UUID",
  }),
  ...
  .or("tagIds", "tagsIds")
  .messages({
    "object.missing": "Tag IDs are required",
  });
