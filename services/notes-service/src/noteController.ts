...
export const getNotesByTag = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const result = await noteService.getNotesByTag(
      userId,
      tagId,
      page,
      limit,
    ...
    return res
      .status(200)
      .json(
        createSuccessResponse(result, "Notes retrieved by tag successfully"),
      );
  },
);
...
export const deleteNote = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res
    .status(200)
    .json(createSuccessResponse(note, "Note deleted successfully"));
});
...
export const restoreNote = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res
    .status(200)
    .json(createSuccessResponse(note, "Note restored successfully"));
});
